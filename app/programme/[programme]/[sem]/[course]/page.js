...
const page = async ({ params }) => {
  ...
  return (
    <main className="p-2 min-h-screen">
      <h1 className="text-3xl mb-4 border-b-2 border-black capitalize font-bold">Course <span className="uppercase">{course}</span></h1>
      <div>
          ...
          <div key={block._id} >
            ...
            <div className="flex flex-col gap-2 ml-4 mb-4">
              {block.units.map((unit) => (
                <a key={unit._id} href={unit?.url} target="_blank" className="w-max">{unit.name}</a>
              ))}
            </div>
          </div>
        ))}
